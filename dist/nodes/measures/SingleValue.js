...
const registerSingleValueMeasureDelegate = ({ className, label, htmlType, }) => {
    Measure_1.default.registerDelegate(className, {
        ...
        builder: (state, { set, attrs }) => {
            ...
            return (React.createElement("div", null,
                ...
                React.createElement("input", { type: htmlType, disabled: !state.enabled }),
                React.createElement("button", { onClick: () => set({ enabled: !state.enabled }) }, "Toggle")));
        },
        menuItem: {
            icon: outline_icons_1.BeakerIcon,
            keywords: `measure single ${className}`,
        },
    });
};
singleValueMeasuresScaffold.forEach((_a) => {
    var props = __rest(_a, []);
    return registerSingleValueMeasureDelegate(props);
});
